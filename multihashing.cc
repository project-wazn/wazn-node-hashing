...
NAN_METHOD(cryptonight_wazn) {
    ...
    switch (variant) {
       case 0:
#if !SOFT_AES && defined(CPU_INTEL)
                cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_INTEL>     (reinterpret_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), reinterpret_cast<uint8_t*>(output), &ctx, 0);
#elif !SOFT_AES && defined(CPU_AMD)
                cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_RYZEN>     (reinterpret_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), reinterpret_cast<uint8_t*>(output), &ctx, 0);
#elif !SOFT_AES && defined(CPU_AMD_OLD)
                cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_BULLDOZER> (reinterpret_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), reinterpret_cast<uint8_t*>(output), &ctx, 0);
#else
                cryptonight_single_hash    <xmrig::CRYPTONIGHT_WAZN, SOFT_AES, xmrig::VARIANT_WAZN1>             (reinterpret_cast<const uint8_t*>(Buffer::Data(target)), Buffer::Length(target), reinterpret_cast<uint8_t*>(output), &ctx, 0);
#endif
                break;
       default:
#if !SOFT_AES && defined(CPU_INTEL)
...
class CCryptonightPicoAsync : public Nan::AsyncWorker {
    ...
    public:
        ...
        CCryptonightPicoAsync(Nan::Callback* const callback, const char* const input, const uint32_t input_len, const int variant)
            ...
            m_ctx->memory = static_cast<uint8_t *>(_mm_malloc(xmrig::CRYPTONIGHT_WAZN_MEMORY, 4096));
        }
        ...
        void Execute () {
            switch (m_variant) {
                case 0:
#if !SOFT_AES && defined(CPU_INTEL)
                         cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_INTEL>     (reinterpret_cast<const uint8_t*>(m_input), m_input_len, reinterpret_cast<uint8_t*>(m_output), &m_ctx, 0);
#elif !SOFT_AES && defined(CPU_AMD)
                         cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_RYZEN>     (reinterpret_cast<const uint8_t*>(m_input), m_input_len, reinterpret_cast<uint8_t*>(m_output), &m_ctx, 0);
#elif !SOFT_AES && defined(CPU_AMD_OLD)
                         cryptonight_single_hash_asm<xmrig::CRYPTONIGHT_WAZN, xmrig::VARIANT_WAZN1, xmrig::ASM_BULLDOZER> (reinterpret_cast<const uint8_t*>(m_input), m_input_len, reinterpret_cast<uint8_t*>(m_output), &m_ctx, 0);
#else
                         cryptonight_single_hash<xmrig::CRYPTONIGHT_WAZN, SOFT_AES, xmrig::VARIANT_WAZN1>                 (reinterpret_cast<const uint8_t*>(m_input), m_input_len, reinterpret_cast<uint8_t*>(m_output), &m_ctx, 0);
#endif
			 break;
                default:
#if !SOFT_AES && defined(CPU_INTEL)
...
NAN_METHOD(cryptonight_wazn_async) {
    if (info.Length() < 2) return THROW_ERROR_EXCEPTION("You must provide at least two arguments.");
...
NAN_MODULE_INIT(init) {
    ...
    Nan::Set(target, Nan::New("cryptonight_wazn").ToLocalChecked(), Nan::GetFunction(Nan::New<FunctionTemplate>(cryptonight_wazn)).ToLocalChecked());
    Nan::Set(target, Nan::New("cryptonight_wazn_async").ToLocalChecked(), Nan::GetFunction(Nan::New<FunctionTemplate>(cryptonight_wazn_async)).ToLocalChecked());
}
